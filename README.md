...
The run is owned by the Gateway and is global to the dashboard: changing
sections, opening another dashboard tab or refreshing the browser does not
cancel it. Every dashboard view continues polling the live metrics until the
run completes or **Cancel simulator** is pressed. A Render service restart ends
an in-memory run.
